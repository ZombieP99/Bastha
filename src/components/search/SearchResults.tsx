...
import { Search, Filter, Star, ShoppingCart, ChevronDown, Heart } from 'lucide-react';
                ...
                {[5, 4, 3].map((rate) => (
                  <label key={rate} className="flex items-center gap-3 cursor-pointer">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-gray-500 text-xs ml-1">{isAr ? 'وأكثر' : '& Up'}</span>
                    </div>
                  </label>
                ))}
              </div>
        ...
        <div className="w-full lg:w-3/4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6 flex items-center justify-between">
            <div className="relative group">
              <button className="flex items-center gap-2 text-sm font-medium text-gray-600 bg-gray-50 px-4 py-2 rounded-lg border border-transparent hover:border-gray-200 transition">
                {dict.search?.sort || 'Sort by'}
                <ChevronDown className="w-4 h-4" />
              </button>
              <div className={`absolute top-full mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20 ${isAr ? 'right-0' : 'left-0'}`}>
          ...
          {filteredProducts.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 text-center border border-gray-100 flex flex-col items-center justify-center">
              ...
              <h3 className="text-xl font-bold text-gray-800 mb-2">{isAr ? 'لم يتم العثور على منتجات' : 'No products found'}</h3>
